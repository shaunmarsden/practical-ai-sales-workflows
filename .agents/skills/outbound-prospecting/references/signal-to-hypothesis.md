# Signal to Hypothesis

A public signal is not evidence of an internal problem. It is a clue worth checking, and it stays a clue until a reply, a call, or another confirmed source actually establishes the problem behind it. Turning a signal into a first-touch message means being explicit about the gap between the two, not quietly treating the signal as though it already proves the case.
...
Prospecting starts from one of two directions, and the evidence checks differ slightly depending on which one is actually true for a given round of targeting:

**Company-first**, when there is a defined account list or a specific market already chosen. The question here is which of the known accounts currently shows a real, checkable signal worth approaching on, not whether the account belongs on the list at all.

**Signal-first**, when accounts are being discovered from a relevant public event, a funding round, a leadership change, a relevant news item, rather than from an existing list. The question here is the reverse: whether an account surfaced this way actually fits the ideal customer profile at all, not just whether the signal itself is real.

Both routes still need a named, reachable buyer with plausible authority and a workable contact route, exactly as the [outbound prospecting skill](../SKILL.md) already requires, before a signal or an account becomes an actual first-touch message. Company-first work that ignores the signal check risks messaging a fitting account with no real hook. Signal-first work that ignores the fit check risks chasing an interesting event at a company that was never going to be a good customer regardless.

## Worked Shape, Not a New Example

This is a structure to apply within the existing [Cedarwell example](cedarwell-example.md), not a new fictional scenario. Read that example for the fuller worked case; apply the six-part structure above to whichever signal is actually driving the approach.
