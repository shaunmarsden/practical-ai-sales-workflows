...
description: Diagnose a sales objection and produce a live-call script, a written reply, or a pipeline decision on what to do next. Use when a prospect has raised a specific concern, pushback, or blocker, whether spoken or written. Do not use for a prospect who has simply gone quiet with no stated objection; use the chase sequence skill for that.
---
...
- The objection as stated, quoted or paraphrased
- What is already known about this person's role, authority, and stage in the process
- Whether this needs a fast spoken answer for a live call, or a considered written reply
- Any objection already raised earlier in this deal, so the same one is not re-litigated from scratch as if it were new

## Identify the Bucket

Before answering, work out what is actually driving the objection, not just its surface wording:
...
- `disqualification`: this genuinely does not fit, not the right conversation
...
- **Isolate**: confirm whether this is the only thing standing in the way, or one of several
- **Reframe**: address the real concern identified in the bucket above, not just the surface wording
- **Ask**: end with a specific question or a concrete next step, never just an explanation

Every objection response ends in one of: the conversation progresses, a dated follow-up is agreed, it moves to a longer nurture, or it is honestly disqualified. Never answer the objection and drift with no next step at all.
...
Read [the Hartwell example](references/hartwell-example.md) for a fictional test, [the Wrenford example](../../../examples/wrenford-objection-input.md) for a harder test of the stop condition above, an objection that is genuinely a contractual question nobody has confirmed, or [the Thornbury example](../../../examples/thornbury-objection-input.md) for a case where the surface wording matches one bucket exactly, and the standard, factually correct answer to that bucket is precisely the wrong move. All three are scored: [Hartwell](../../../evaluations/hartwell-objection-review.md), [Wrenford](../../../evaluations/wrenford-objection-review.md) and [Thornbury](../../../evaluations/thornbury-objection-review.md). A deliberately [ambiguous objection](../../../evaluations/hartwell-objection-ambiguous-test.md) was then run nine times, three each across three models: ChatGPT and Gemini named the same primary driver every time, and Claude promoted a different one in each of its three runs, so treat one diagnosis as a starting point rather than a settled answer. On this job, [Which AI Mistakes Actually Get Through](../../../guides/which-ai-mistakes-get-through.md) records an invented price of nine hundred pounds, an invented cost question, quarter and fortnight, and the rebuttal a case explicitly warned against, proposed as though it were a neutral question.

[Sales Proof Bench](https://github.com/shaunmarsden/sales-proof-bench) gives the same job to several models with no method attached. On its fictional [Osmond case](https://github.com/shaunmarsden/sales-proof-bench/blob/main/cases/osmond-objection-diagnosis-case.md), every run read the objection as more than one thing, and the failures were in what got added along the way: one invented a price figure that appears nowhere in the source, and another proposed the phased rollout the case explicitly warns against instead of asking a neutral question. Those runs used the bench's bare task request rather than this skill, so they show what the job looks like without a method rather than testing this one.
