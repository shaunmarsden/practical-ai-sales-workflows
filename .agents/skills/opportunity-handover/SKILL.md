...
For a fictional test, read [the Hartwell example](references/hartwell-example.md), and the [honest evaluation](../../../evaluations/hartwell-opportunity-handover-review.md) for how it scored. Reaching that clean run took several discarded attempts and one failed fix, set out in the [instruction change and regression history](../../../evaluations/opportunity-handover-instruction-change-history.md), and there is a separate [real use finding](../../../evaluations/opportunity-handover-real-use-finding.md). That failed fix is the case [Which AI Mistakes Actually Get Through](../../../guides/which-ai-mistakes-get-through.md) uses to show that a general instruction is not a mechanism: the reference ledger and the audit pass above are the mechanism.
