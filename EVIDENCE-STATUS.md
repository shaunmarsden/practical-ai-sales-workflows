...
Eleven of them. Two ran the same input repeatedly to measure how much a score moves on its own, eight compared an instruction with and without one change, and one compared two versions of a scenario rather than of an instruction. The third of those eight reused runs from the other two as its baseline rather than running its own, and the last six were scored without knowing which version each run came from.
