...
Nine of them. Two ran the same input repeatedly to measure how much a score moves on its own, six compared an instruction with and without one change, and one compared two versions of a scenario rather than of an instruction. The third of those six reused runs from the other two as its baseline rather than running its own, and the last four were scored without knowing which version each run came from.
