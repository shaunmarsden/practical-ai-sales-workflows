...
- **Keep testing against real work as it comes up.** Not a batch exercise, and not blocking anything else: when a real objection, a real pipeline check, or a real anything else comes up, run the matching workflow on it and see honestly whether it held up. Twelve verticals have real evidence behind them now. Two, hand over an opportunity and move a stalled decision, had no real material available as of 24 July 2026; check back on 7 August 2026 for whether a genuine case has turned up for either. Prepare for a sales call is deliberately excluded from this gap, since it is used live in a chat rather than as something tested and recorded this way. The controlled first round is defined in [Usability Test: Round One](evaluations/usability-test-round-one.md), with one [session log](templates/usability-test-session-log.md) per participant.

## Done Recently

- **Made the README and the sixteen recipe cards a clearer shop window, acting on an external AI critique of the README's readability.** Checked each of the critique's specific claims (link counts, section sizes, the Patreon badge's position) against the actual files before acting on any of them; all held up. The README's Choose a Sales Problem section now features six common starting problems with one link each, the rest reached via [Browse all sixteen recipe cards](recipes/README.md) rather than sixteen equally-weighted sections. Each of the sixteen [recipe cards](recipes/README.md) had its own version of the same problem, an Open row listing two to five links with no priority between them; each now bolds the one link its own closing line already treats as the fuller method (the workflow, or the skill where no separate workflow file exists) and demotes the rest under "Also:". Verified the actual rendering on github.com across every link-count variant present, not just the markdown source. Nothing removed, no files deleted.
