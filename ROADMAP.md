...
- **Three more diagrams, generated then hand-finished**: an [evidence-labels diagram](assets/diagrams/how-this-repository-labels-evidence.svg) for [METHODOLOGY.md](METHODOLOGY.md), a [growth-path diagram](assets/diagrams/get-more-from-your-ai.svg) for [Get More From Your AI](guides/get-more-from-your-ai.md), and an upgraded version of the [README's "How I Approach It" diagram](README.md), replacing three plain prose or Mermaid-box sections after direct reader feedback that the repository reads as text heavy. Built using [the reusable Gemini prompt](templates/gemini-visual-style-prompt.md) and checked word for word against the supplied wording before use, following the same discipline as the earlier sales-cycle and where-to-start visuals: two of six generation attempts across the three diagrams came back on a completely different, invented topic and were discarded outright, and one came back on-topic but with extra captions and a footer sentence never supplied, which were stripped directly from the SVG source rather than accepted, since the house rule is not to let an AI tool add or rewrite supplied wording without permission. A second round of direct feedback was that the first pass looked flat next to the existing sales-cycle image, so the icons were hand-redesigned as richer, two-element compositions (matching the reference's magnifying-glass-and-person, folder-and-document style) rather than re-prompting Gemini again given how unreliable it had already been. That pass also caught its own mistake before anyone else saw it: the growth-path diagram's automation stage first used a small robot face, which directly breaks the visual style prompt's own "no robots" rule, and was replaced with a gear and caution triangle.
