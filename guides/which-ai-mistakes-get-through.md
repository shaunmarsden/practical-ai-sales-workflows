...
- **One prompt here missed a conflict its own skill caught, and two tests have failed to explain why.** A promised delivery date sat inside a period of stated leave and [the prompt's run never mentioned it](../evaluations/hartwell-chase-prompt-review.md). This page originally said the cause was unknown. It still is: [widening the instruction](../evaluations/chase-stale-date-test.md) was rejected when four of six runs of the published prompt named the commitment anyway, and [naming the weekday in the scenario](../evaluations/chase-weekday-rerun.md), which the same test showed was needed to make the conflict checkable at all, moved it to one of six. Both candidate causes are eliminated and the requirement still goes unmet five times in six.
