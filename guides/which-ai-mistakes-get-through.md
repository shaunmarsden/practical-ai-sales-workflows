...
- **The stacked figure ran four times in six before there was anything to stop it.** It had already cost a hallucination-risk mark on a published record, on a £65,520 version of the same mistake, and it took [nineteen more runs](../evaluations/business-case-stacked-figure-test.md) to establish that a guardrail fixed it.
- **One prompt here missed a conflict its own skill caught.** A promised delivery date sat inside a period of stated leave, and [the prompt's run never mentioned it](../evaluations/hartwell-chase-prompt-review.md). Whether the wording was too narrow or the run was unlucky is still not known.
- **An instruction that has never once been followed.** Both the business case skill and its prompt ask for three applied examples. Across six runs the count was one, zero, zero, one, zero and one. Nobody noticed until the sixth run, and the honest reading is that the instruction may be unrealistic rather than ignored.
