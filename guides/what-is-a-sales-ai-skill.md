...
Most skills here use the same fictional Hartwell conversation, so none of them contain real customer or employer information. Four use a different invented company, because Hartwell would not fit what they need: outbound prospecting uses Cedarwell, since outbound happens before any call exists; the fit and limitations review uses Kellow Distribution, which raises several use cases at once; real blocker diagnosis uses Rowcastle Group, where a call gains an unplanned attendee; and identify buyer indecision uses Apex Logistics, a late-stage deal that keeps slipping. Three work across a whole invented dataset rather than one conversation: the pipeline evidence review, the CRM hygiene review and the objection pattern review. Building a business case uses Hartwell and adds a second Bramfield Insurance Group scenario. Every company, person and figure in all of them is invented.
...
- [Real Blocker Diagnosis](../.agents/skills/real-blocker-diagnosis/SKILL.md): checks whether the person raising a concern is actually the decision-maker, and whether the stated objection is the real one or standing in for something unsaid
...
- [Opportunity Handover](../.agents/skills/opportunity-handover/SKILL.md): prepares an evidence-led handover so the person taking over a live deal knows what is confirmed and what is not
...
- [Pipeline Evidence Review](../.agents/skills/pipeline-evidence-review/SKILL.md): checks whether the stages, close dates and next steps in a pipeline are actually supported by evidence, not just recorded in the CRM
- [CRM Hygiene Review](../.agents/skills/crm-hygiene-review/SKILL.md): audits a CRM export for duplicates, missing fields, stale records and close dates that do not fit their stage
