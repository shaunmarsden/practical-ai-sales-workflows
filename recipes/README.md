...
Seventeen sales jobs, one page each. Every card covers the same seven things: what it helps with, what you need, what you'll get, the prompt to paste, what the AI cannot decide, what you must check, and what to do next. Sixteen of the seventeen carry the prompt on the card itself, so you never need to open anything else to use one. Build a Business Case has no standalone prompt yet, only a skill, and says so at the top rather than claiming otherwise.
