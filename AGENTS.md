...
- [Build an Approval-Gated Sales Copilot](guides/build-an-approval-gated-sales-copilot.md), the composition method for an orchestrating agent that chooses between several bounded workflows, based on Shaun's own private sales copilot, with a reusable vendor-neutral template, a fictional multi-source test and a scored evaluation
