...
- Every example file should say plainly, near the top, that it is fictional and which skill or workflow it was created to test. **Name the job, not the difficulty.** What the scenario hinges on belongs below the re-run line with the answer key, or in the evaluation, because the re-run warning tells a reader to copy everything above that line. An earlier version of this rule asked for "what it was created to test", and fifteen published inputs ended up foreshadowing their own answer key in the one part of the file a re-runner is told to keep.
- The same applies to a skill's own pointer to a test scenario. The skill gets pasted into the model alongside the input, so a footer saying what the harder test hinges on hands the answer over as surely as the input would. Say that a scenario is harder, and leave what makes it harder to its evaluation.
- The [repository checks](.github/scripts/repo_checks.py) enforce that examples contain the word "fictional". They cannot enforce the rule above: two mechanical proxies were tried, blockquote length and shared phrasing between the disclosure and the answer key, and neither separated the leaking files from the clean ones.
