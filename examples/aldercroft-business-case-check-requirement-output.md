...
**One edit was made, and it is the finding this run threw up.** The raw output contained twenty-three em dashes, which this repository's own style rule and CI check forbid, so they were converted to commas, colons and full stops. The choice of replacement in each place is mine, and in eight places a bare comma would have left a splice so a full stop was used instead. No words were added, removed or reordered. The Build a Business Case skill does not tell the model to avoid em dashes, and all six runs in the test produced between thirteen and twenty-three of them. The [chase sequence prompt](../templates/chase-sequence-prompt.md), which does carry that rule, produced none. See the [test](../evaluations/business-case-check-requirement-test.md) for what that means for every other published output here.
