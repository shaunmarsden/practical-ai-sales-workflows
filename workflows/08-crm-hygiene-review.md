...
5. Stale records, including ones that look complete but have not moved in a long time
6. Close dates that do not fit their recorded stage
7. An explicit note on records with no issue found, so the review does not read as a list of only problems
...
### 2. Separate Confident Findings From Uncertain Ones
...
### 4. Stay Out of the Stage-Accuracy Question
...
Every finding is a suggestion. Merging duplicates, filling in a missing field, or reassigning an owner all stay with a person. Note where a scoring threshold, such as what counts as "stale," was used, and make clear it is illustrative, not a universal rule.

### 6. Call the Clean Records Clean
